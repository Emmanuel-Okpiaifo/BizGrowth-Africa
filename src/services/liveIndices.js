...
export async function fetchIndexTimeseries(symbol, days = 120) {
	// Trading Economics historical
	const creds = teCreds();
	...
	if (!res.ok) throw new Error(`Index timeseries failed: ${res.status}`);
	const data = await res.json();
	// Expect array with objects that include 'Date' and 'Close' (varies; adjust if needed)
	const labels = [];
	const series = [];
...
export async function fetchIndexLatest(symbol) {
	const creds = teCreds();
	if (!creds) throw new Error("Trading Economics credentials not configured");
	const url = `https://api.tradingeconomics.com/markets/symbol/${encodeURIComponent(symbol)}?c=${creds}&format=json`;
	const res = await fetch(url);
	if (!res.ok) throw new Error(`Index latest failed: ${res.status}`);
	const data = await res.json();
