...
const MembershipForm = ({ 
  ...
  const [referralToast, setReferralToast] = useState({ show: false, message: '' });
  ...
  // Get referrer ID from URL or localStorage
  const getReferrerId = () => {
  ...
  // Initialize user and referrer IDs
  useEffect(() => {
    setUserId(getOrCreateUserId());
    setReferrerId(getReferrerId());
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      referrerId: referrerId || getReferrerId(),
  ...
  const handleCopyReferralLink = async () => {
    ...
    u.hash = '';
    u.searchParams.set('ref', userId || getOrCreateUserId());
    
    try {
      await navigator.clipboard.writeText(u.toString());
      setReferralToast({ show: true, message: 'Link copied' });
      setTimeout(() => {
        setReferralToast({ show: false, message: '' });
      }, 2000);
    } catch {
      setReferralToast({ show: true, message: u.toString() });
      setTimeout(() => {
        setReferralToast({ show: false, message: '' });
      }, 4000);
    }
  ...
  return (
    <div className="membership-form-wrapper">
      <div className="waitlist-container">
        ...
        >
          Join the BizGrowth Africa waitlist to unlock tenders, grants, and trusted business news at launch. Membership notices arrive first.
        </p>
        <form
          ref={(el) => {
            formRef.current = el;
        ...
        {showDisclaimer && (
          ...
          >
            Become a member today and receive early access, curated tenders & grants, and launch updates.
          </p>
        )}
        
        {showReferralCard && (
          <div 
            className="referral-card animate-on-scroll"
            ref={(el) => {
              if (el && !elementsRef.current.includes(el)) {
                elementsRef.current.push(el);
              }
          ...
          >
            ...
            <p className="referral-copy">Move up 5 spots for each friend who joins from your link.</p>
            <button
